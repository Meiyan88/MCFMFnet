...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.downf1 = Upsample(scale_factor=[0.5, 0.25, 0.25], mode='trilinear')
        self.downf2 = Upsample(scale_factor=[0.25, 0.25, 0.25], mode='trilinear')
        self.downf3 = Upsample(scale_factor=[0.25, 0.25, 0.25], mode='trilinear')
        if self.upscale_logits:
    ...
    def forward(self, x):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            if not self.convolutional_pooling:
                x = self.td[d](x)
        # cc = self.downf3(skips_3_to_5[2]), self.downf2(skips_3_to_5[1]), self.downf1(skips_3_to_5[0])
        fusion_feat = self.ncd_fusion(self.downf3(skips_3_to_5[2]), self.downf2(skips_3_to_5[1]), self.downf1(skips_3_to_5[0]))
        ...
        for u in range(len(self.tu)):
            if u > 2:
                ...
                refinement_skips = self.RAG[u](skips[-(u + 1)], x)
                x = self.tu[u](x)
